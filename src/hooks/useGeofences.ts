...
import { NavixyService, NavixyTrackerState } from '../services/navixy';
...
export function useGeofences(
    ...
    const loadZones = useCallback(async () => {
        ...
        try {
            ...
            const region = sessionKey.includes('ZM') ? 'ZM' : 'TZ'; // Crude check, but session keys should belong to a region

            // Fetch all share tokens for this region from Supabase
            const { data: shareTokens } = await supabase
                .from('geofence_shares')
                .select('navixy_zone_id, share_token')
                .eq('region', region);

            const shareTokenMap: Record<number, string> = {};
            shareTokens?.forEach((st: any) => {
                shareTokenMap[Number(st.navixy_zone_id)] = st.share_token;
            });
